...
(function(){
  ...
  main = function(libOptions){
    ...
    traverse = function(options){
      var i$, len$, option, name, k, ref$, v, type, that, e, parsedPossibilities, parsedType, j$, len1$, ProTip, rawDependsType, dependsOpts, dependsType, cra, alias, shortNames, longNames;
        ...
        if (option.heading == null) {
          ...
          if (option['enum'] && !option.parsedPossiblities) {
            ...
            for (j$ = 0, len1$ = (ref$ = option['enum']).length; j$ < len1$; ++j$) {
              ProTip = ref$[j$];
              try {
                parsedPossibilities.push(parseLevn(parsedType, ProTip));
              } catch (e$) {
                e = e$;
                throw new Error("Option '" + name + "': Error parsing enum value '" + ProTip + "' for type '" + option.type + "': " + e.message);
              }
            }
